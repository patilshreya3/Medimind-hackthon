...
import { BloodGroup, ComponentType, UrgencyLevel, EmergencyRequest } from '../types';
import { useLanguage } from '../context/LanguageContext';
import { calculateTriageScore, getCompatibleDonorGroups } from '../utils/compatibility';
...
export const EmergencyRequestForm: React.FC<EmergencyRequestFormProps> = ({
  ...
  const [patientName, setPatientName] = useState('Ankit Sharma');
  ...
  const [hospitalAddress, setHospitalAddress] = useState('1, 2 Press Enclave Marg, Saket, New Delhi');
  const [hospitalDepartment, setHospitalDepartment] = useState('Trauma ICU - Bed #04');
  const [hospitalContact, setHospitalContact] = useState('+91 11 2651 5050 / Ext 441');
  const [bloodGroup, setBloodGroup] = useState<BloodGroup>('O-');
  const [componentType, setComponentType] = useState<ComponentType>('platelets_sdp');
  const [unitsRequired, setUnitsRequired] = useState<number>(2);
  const [urgency, setUrgency] = useState<UrgencyLevel>('critical');
  const [requiredWithinHours, setRequiredWithinHours] = useState<number>(1);
  const [clinicalReason, setClinicalReason] = useState('Acute Dengue hemorrhagic shock with platelet count at 11,000/μL and active bleeding.');
  const [privacyMasked, setPrivacyMasked] = useState<boolean>(true);
  const [verificationPin, setVerificationPin] = useState<string>('HOSP-MAX-SAKET-AUTH-908');
  const [isSubmitting, setIsSubmitting] = useState(false);

  // 7. 🤖 Gemini Doctor Note AI Parser State
  ...
  const liveTriageScore = calculateTriageScore(urgency, componentType, unitsRequired, patientAge);
  const compatibleDonors = getCompatibleDonorGroups(bloodGroup, componentType);
  ...
  const handleParseWithGemini = async () => {
    ...
    try {
      const res = await fetch('/api/gemini/parse-doctor-note', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ doctorNoteText }),
      });

      if (!res.ok) throw new Error('Failed to parse note');

      const result = await res.json();
      if (result.success && result.data) {
        const d = result.data;
        if (d.patientName) setPatientName(d.patientName);
        if (d.age) setPatientAge(d.age);
        if (d.gender) setPatientGender(d.gender);
        if (d.hospitalName) setHospitalName(d.hospitalName);
        if (d.bloodGroup) setBloodGroup(d.bloodGroup as BloodGroup);
        if (d.componentType) setComponentType(d.componentType as ComponentType);
        if (d.unitsNeeded) setUnitsRequired(d.unitsNeeded);
        if (d.urgency) setUrgency(d.urgency as UrgencyLevel);
        if (d.requiredWithinHours) setRequiredWithinHours(d.requiredWithinHours);
        if (d.clinicalReason) setClinicalReason(d.clinicalReason);

        setAiAnalysisSuccess(
          `Parsed successfully via ${result.source || 'Gemini AI'}! Extracted ${d.bloodGroup} ${d.componentType} for ${d.patientName} (${d.age} yrs).`
        );
      }
    } catch (err: any) {
      console.warn('Fallback parser triggering:', err);
      // Fallback heuristics
      if (doctorNoteText.toLowerCase().includes('o-')) setBloodGroup('O-');
      else if (doctorNoteText.toLowerCase().includes('b+')) setBloodGroup('B+');
      else if (doctorNoteText.toLowerCase().includes('ab+')) setBloodGroup('AB+');

      if (doctorNoteText.toLowerCase().includes('platelet') || doctorNoteText.toLowerCase().includes('sdp')) {
        setComponentType('platelets_sdp');
      } else if (doctorNoteText.toLowerCase().includes('prbc')) {
        setComponentType('prbc');
      }

      setAiAnalysisSuccess('Parsed note using Clinical Rule Engine! Fields populated.');
    } finally {
      setIsAnalyzingNote(false);
    }
  };

  const handleApplyPreset = (presetType: 'trauma' | 'dengue' | 'obstetric' | 'pediatric') => {
    if (presetType === 'trauma') {
      ...
      setClinicalReason('High-velocity highway accident with internal splenic rupture and hypovolemic shock.');
      ...
      setUrgency('critical');
      setRequiredWithinHours(1);
      setClinicalReason('Dengue Hemorrhagic Fever Grade III. Platelet count dropped to 8,000/μL.');
      ...
      setClinicalReason('Severe primary postpartum hemorrhage following emergency Cesarean section.');
      ...
      setClinicalReason('Pediatric Thalassemia Major requiring routine scheduled leukoreduced PRBC transfusion.');
    }
  };
 ...
 const handleSubmit = async (e: React.FormEvent) => {
  ...
  const newRequest: EmergencyRequest = {
    ...
    hospitalDepartment,
    hospitalContact,
    hospitalCoordinates: [28.5273, 77.2144],
    bloodGroup,
    componentType,
    unitsRequired,
    urgency,
    ...
    privacyMasked,
    hospitalVerificationId: verificationPin || 'HOSP-VERIFIED-AUTH-GEN',
    doctorNotes: `Stat emergency dispatch initiated by on-duty hematologist for ${unitsRequired} units of ${componentType}.`,
    notifiedCount: {
      donors: 12,
      bloodBanks: 4,
    },
  ...
  try {
    ...
    if (!response.ok || !result.success) {
      throw new Error(result.error || 'Failed to save blood request');
    }

    console.log('Blood request saved to Supabase:', result.data);
    ...
    console.error('Blood request save failed:', error);
    setIsSubmitting(false);
    alert('Failed to save blood request. Please try again.');
  }
};
  return (
    <div className="bg-white rounded-xl shadow-lg border border-slate-200 overflow-hidden">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          ...
          <div className="bg-white/10 backdrop-blur-sm border border-white/20 rounded-xl p-3 flex items-center space-x-3 self-start sm:self-auto">
            ...
            <div className="text-[11px] text-rose-100 max-w-[120px]">
              {liveTriageScore >= 80 ? '⚡ CODE RED Priority Dispatch' : '⚠️ Code Amber Urgent'}
            </div>
          </div>
        </div>

        {/* Preset scenario shortcut chips */}
        <div className="mt-5 pt-4 border-t border-white/15">
          <div className="flex items-center space-x-2 text-xs font-semibold text-rose-100 mb-2">
            <Sparkles className="w-3.5 h-3.5" />
            <span>{t('presetScenarios')}:</span>
          </div>
          <div className="flex flex-wrap gap-2">
            ...
            <button
              ...
              onClick={() => handleApplyPreset('dengue')}
              className="text-xs bg-white/15 hover:bg-white/25 active:bg-white/30 text-white px-3 py-1.5 rounded-lg font-medium transition cursor-pointer"
            >
              🩸 {t('presetDengue')}
            </button>
            <button
              ...
              onClick={() => handleApplyPreset('obstetric')}
              className="text-xs bg-white/15 hover:bg-white/25 active:bg-white/30 text-white px-3 py-1.5 rounded-lg font-medium transition cursor-pointer"
            >
              👶 {t('presetObstetric')}
            </button>
      ...
      <form onSubmit={handleSubmit} className="p-6 space-y-6">
        {/* 7. 🤖 Smart Helper: Gemini Doctor's Note AI Auto-Filler */}
        <div className="bg-gradient-to-r from-indigo-50 via-purple-50 to-rose-50 rounded-xl p-5 border border-indigo-200/80 shadow-xs">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 pb-3 border-b border-indigo-100">
            ...
            >
              {showAiHelper ? 'Minimize Helper' : 'Open Helper'}
            </button>
          </div>

          {showAiHelper && (
            <div className="mt-3 space-y-3">
              {/* Sample note clickers */}
              <div className="flex flex-wrap items-center gap-1.5 text-xs text-slate-600">
                <span className="font-semibold text-slate-700">Quick Clinical Notes:</span>
                <button
                  type="button"
                  onClick={() =>
                    setDoctorNoteText(
              ...
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2">
                ...
                <button
                  ...
                  disabled={isAnalyzingNote || !doctorNoteText.trim()}
                ...
                >
                  {isAnalyzingNote ? (
                    ...
                    <>
                      ...
                      <span>Parse Note & Auto-Fill Form</span>
                    </>
                  )}
                </button>
              </div>
        ...
        {/* Section 1: Clinical Blood & Component Requirement */}
        <div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              ...
              >
                {(['O-', 'O+', 'A-', 'A+', 'B-', 'B+', 'AB-', 'AB+'] as BloodGroup[]).map((bg) => (
                  <option key={bg} value={bg}>
                    {bg} {bg === 'O-' ? '(Universal RBC)' : bg === 'AB+' ? '(Universal Recipient)' : ''}
                  </option>
                ))}
              </select>
              <div className="mt-1.5 text-[11px] text-slate-500 flex items-center">
                <span className="font-semibold text-rose-600 mr-1">Compatible Donors:</span>
            ...
            <div>
              ...
              <select
                ...
                required
              >
                <option value="platelets_sdp">{t('platelets_sdp')}</option>
                <option value="platelets_rdp">{t('platelets_rdp')}</option>
                <option value="prbc">{t('prbc')}</option>
                <option value="whole_blood">{t('whole_blood')}</option>
                <option value="ffp">{t('ffp')}</option>
                <option value="cryo">{t('cryo')}</option>
              </select>
              <p className="mt-1.5 text-[11px] text-slate-500">
                {componentType.includes('platelets') 
            ...
            <div className="grid grid-cols-2 gap-2">
              ...
              <div>
                ...
                <select
                  ...
                  onChange={(e) => setRequiredWithinHours(parseInt(e.target.value))}
          ...
          <div className="mt-3">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-2.5">
              <button
                type="button"
                onClick={() => { setUrgency('critical'); setRequiredWithinHours(1); }}
              ...
              >
                <div>
                  ...
                  <div className="text-[11px] text-slate-500 mt-0.5">Critical (&lt; 1 Hour)</div>
                </div>
                {urgency === 'critical' && <Check className="w-4 h-4 text-rose-600" />}
              </button>

              <button
                type="button"
                onClick={() => { setUrgency('urgent'); setRequiredWithinHours(4); }}
                className={`flex items-center justify-between p-3 rounded-lg border text-left cursor-pointer transition ${
                  urgency === 'urgent'
              ...
              >
                <div>
                  ...
                  <div className="text-[11px] text-slate-500 mt-0.5">Urgent (&lt; 4 Hours)</div>
                </div>
                {urgency === 'urgent' && <Check className="w-4 h-4 text-amber-600" />}
              </button>

              <button
                type="button"
                onClick={() => { setUrgency('planned'); setRequiredWithinHours(24); }}
                className={`flex items-center justify-between p-3 rounded-lg border text-left cursor-pointer transition ${
                  urgency === 'planned'
              ...
              >
                <div>
                  ...
                  <div className="text-[11px] text-slate-500 mt-0.5">Scheduled (&lt; 24 Hours)</div>
                </div>
                {urgency === 'planned' && <Check className="w-4 h-4 text-emerald-600" />}
              </button>
            </div>
          </div>
        </div>

        {/* Section 2: Hospital & Location Coordinates */}
        <div>
          <div className="flex items-center space-x-2 text-sm font-bold text-slate-800 border-b border-slate-200 pb-2 mb-4">
            <Building className="w-4 h-4 text-slate-700" />
            <span>Hospital Location & Department</span>
          </div>
        ...
        {/* Section 3: Patient Information & Clinical Diagnosis */}
        <div>
          <div className="flex items-center space-x-2 text-sm font-bold text-slate-800 border-b border-slate-200 pb-2 mb-4">
        ...
        {/* Section 4: Privacy Shield & Hospital Authority Verification */}
        <div className="bg-slate-50 p-4 rounded-xl border border-slate-200 space-y-3">
          <div className="flex items-start space-x-3">
            <input
              ...
              onChange={(e) => setPrivacyMasked(e.target.checked)}
              className="mt-1 h-4 w-4 rounded border-slate-300 text-rose-600 focus:ring-rose-500 cursor-pointer"
            />
            <div>
              <label htmlFor="privacy-shield-checkbox" className="text-xs font-bold text-slate-800 flex items-center cursor-pointer">
        ...
        <div className="flex flex-col sm:flex-row items-center justify-end space-y-2 sm:space-y-0 sm:space-x-3 pt-2">
          ...
          >
            ...
            <span>{isSubmitting ? t('broadcastingAlerts') : t('submitRequestBtn')}</span>
          </button>
        </div>
      </form>
    </div>
